...
#include "WiFiS3.h" // Include the WiFiS3 librar y
...
long parseISO8601(const char *dateTime) // Function to parse ISO8601 date time (since c does not have a built-in function I created one)
{
  int year, month, day, hour, minute, second; // Variables to store the year, month, day, hour, minute, and second
  sscanf(dateTime, "%4d-%2d-%2dT%2d:%2d:%2dZ", &year, &month, &day, &hour, &minute, &second); // Parse the date time

  struct tm timeinfo; // Create an instance of the tm struct
  timeinfo.tm_year = year - 1900; // Set the year (since tm_year is years since 1900) 
  timeinfo.tm_mon = month - 1; // Set the month (since tm_mon is months since January)
  timeinfo.tm_mday = day; // Set the day
  timeinfo.tm_hour = hour; // Set the hour 
  timeinfo.tm_min = minute; // Set the minute
  timeinfo.tm_sec = second; // Set the second

  time_t t = mktime(&timeinfo); // Convert the tm struct to a time_t value
  return (long)t; // Return the time_t value as a long 
}
...
void setup()
{
  ...
  delay(5000); 
  if (WiFi.status() == WL_NO_MODULE) // Check if the WiFi module is not present
  {
  ...
  if (fv < WIFI_FIRMWARE_LATEST_VERSION)  // Check if the firmware version is less than the latest version
  {
  ...
  RTCTime timeToSet = RTCTime(unixTime); // Create an instance of the RTCTime class
  RTC.setTime(timeToSet); // Set the RTC time

  RTCTime currentTime; // Create an instance of the RTCTime class
